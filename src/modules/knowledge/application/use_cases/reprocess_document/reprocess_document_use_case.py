...
class ReprocessDocumentUseCase:
    ...
    async def execute(
        ...
        max_retries = 5
        last_err: DomainError | None = None
        ...
        for attempt in range(max_retries):
            ...
            if request.document_id not in kb.documents:
                return Err(
                    DomainError(
                        f"Document {request.document_id} not found in Knowledge Base "
                        f"{request.kb_id}",
                        code="NOT_FOUND",
                    )
                )
        ...
        return Err(last_err or DomainError("Falha de concorrência ao reprocessar documento"))
